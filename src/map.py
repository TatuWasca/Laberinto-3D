...
class Map:
    ...
    def get_spawns(self):
        ...
        spawns = list(filter_spawns(self.exit_spawn, spawns, self.map_width, self.map_height))
        ...
        spawns = list(filter_spawns(self.player_spawn, spawns, self.map_width, self.map_height))

        # Get key spawn
        key_spawn = choice(spawns)
        self.key_spawn = float(Decimal(key_spawn[0]) + Decimal('0.5')), float(Decimal(key_spawn[1]) + Decimal('0.5'))
        spawns = list(filter_spawns(self.key_spawn, spawns, self.map_width, self.map_height))
